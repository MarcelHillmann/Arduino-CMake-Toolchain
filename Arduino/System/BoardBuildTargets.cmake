...
function(target_link_arduino_libraries target_name)
    ...
    foreach (_arg IN LISTS ARGN)
        message(VERBOSE "target_link_arduino_libraries: ARGN ${_arg}")
        list(FIND keywords "${_arg}" _idx)
        message(VERBOSE "target_link_arduino_libraries: ARGN ${_arg} = '${_idx}'")
    ...
    foreach (_keyword IN LISTS keywords)
        message(VERBOSE "target_link_arduino_libraries: keywords ${_keyword} => ${_list_${_keyword}_present}")
        if (${_list_${_keyword}_present})
            message(VERBOSE "target_link_arduino_libraries: keywords ${_list_${_keyword}}")
        endif ()
    endforeach ()
    ...
    message(VERBOSE "target_link_arduino_libraries: _index_local_libraries(${_local_namespace} '_is_indexed')")
    ...
    message(VERBOSE "target_link_arduino_libraries: NOT TARGET '${target_name}'")
    if (NOT TARGET "${target_name}")
        message(VERBOSE "target_link_arduino_libraries: _map_libs_to_lib_names(${target_name})")
        _map_libs_to_lib_names(target_name)
        message(VERBOSE "target_link_arduino_libraries: _map_libs_to_lib_names(target_name) = ${target_name}")
        string(MAKE_C_IDENTIFIER "${target_name}" _target_id)
        message(VERBOSE "target_link_arduino_libraries: target_get_arduino_lib('_arduino_lib_${_target_id}' _ard_lib_name)")
    ...
    message(VERBOSE "target_link_arduino_libraries: _list_DEFAULT ${_list_DEFAULT}")
    if (_list_DEFAULT)
        set(empty_list)
        message(VERBOSE "target_link_arduino_libraries: _list_DEFAULT => ${_list_DEFAULT}")
    ...
    foreach (_link_type IN ITEMS PRIVATE PUBLIC INTERFACE)
        message(VERBOSE "target_link_arduino_libraries: _link_type => ${_link_type}")
        message(VERBOSE "target_link_arduino_libraries:  if (${_list_${_link_type}})")
        if (_list_${_link_type})
            set(empty_list)
            message(VERBOSE "target_link_arduino_libraries: _map_libs_to_lib_names(${_list_${_link_type}})")
            _map_libs_to_lib_names(_list_${_link_type})
            message(VERBOSE "target_link_arduino_libraries: _link_ard_lib_list('${target_name}' '${_list_${_link_type}}' '${_link_type}' empty_list empty_list)")
...
function(target_enable_arduino_upload target)
    ...
    message(STATUS "${KEY} _scripts_dir=${_scripts_dir}")
    set(_app_targets_dir "${CMAKE_BINARY_DIR}/.app_targets")
    message(STATUS "${KEY} _app_targets_dir=${_app_targets_dir}")
    set(_bin_dir "$<TARGET_PROPERTY:${target},BINARY_DIR>")
    message(STATUS "${KEY} _bin_dir=${_bin_dir}")
    set(_src_dir "$<TARGET_PROPERTY:${target},SOURCE_DIR>")
    message(STATUS "${KEY} _src_dir=${_src_dir}")
    ...
    message(STATUS "${KEY} _app_info=${_app_info}")
    message(STATUS "${KEY} file(GENERATE OUTPUT '${_app_targets_dir}/${target}.cmake' CONTENT '${_app_info}')")
    ...
    message(STATUS "${KEY} find_source_files('${ARDUINO_GENERATE_DIR}/sketch' app_sources RECURSE)")
    find_source_files("${ARDUINO_GENERATE_DIR}/sketch" app_sources RECURSE)
    message(STATUS "${KEY} NOT ${app_source}")
    ...
    while (_idx LESS _num_rules)
        ...
        math(EXPR _idx "${_idx}+1")
        message(STATUS "${KEY} EXISTS '${_scripts_dir}/${_script}'")
        if (EXISTS "${_scripts_dir}/${_script}")
            message(STATUS "${KEY} add_custom_command(TARGET '${target}' ${_type} COMMAND ${CMAKE_COMMAND} ARGS -D 'ARDUINO_BUILD_PATH=${_bin_dir}' -D " ARDUINO_BUILD_SOURCE_PATH=${_src_dir}" -D 'ARDUINO_BUILD_PROJECT_NAME=${target}' -P '${_scripts_dir}/${_script}' COMMENT '${_comment}' VERBATIM")
    ...
    while (_idx LESS _num_tools)
        ...
        math(EXPR _idx "${_idx}+1")
        message(STATUS "${KEY} Tool: EXISTS '${_scripts_dir}/${_script}'")
        if (EXISTS "${_scripts_dir}/${_script}")
            message(STATUS "${KEY} Tool: NOT EXISTS '${_tool_target}'")
            if (NOT TARGET ${_tool_target})
                message(STATUS "${KEY} Tool: add_custom_target('${_tool_target}' ${CMAKE_COMMAND} ARGS "
            ...
            message(STATUS "${KEY} add_custom_target('${_tool_target}-${target}' ${CMAKE_COMMAND} ARGS "
                    "-D 'ARDUINO_BUILD_PATH=${_bin_dir}' "
                    "-D 'ARDUINO_BUILD_SOURCE_PATH=${_src_dir}' "
                    "-D 'ARDUINO_BUILD_PROJECT_NAME=${target}' "
                    "-D 'MAKE_TARGET=${_tool_target}' "
                    "-P '${_scripts_dir}/${_script}' "
                    "COMMENT '${_comment} ${target}' VERBATIM)")
...
function(find_arduino_library lib return_lib_path)
    ...
    set(_flag_options NO_DEFAULT_PATH
            QUIET
            EXCLUDE_LIB_NAMES
            EXCLUDE_INCLUDE_NAMES)

    set(_one_arg_options LIBNAME_RESULT)
    set(_multi_arg_options HINTS PATHS)

    cmake_parse_arguments(parsed_args "${_flag_options}" "${_one_arg_options}" "${_multi_arg_options}" ${ARGN})

    message(VERBOSE "find_arduino_library: ${lib} ${parsed_args}")
    ...
    set(_hint_paths ${parsed_args_HINTS} ${parsed_args_PATHS})
    message(VERBOSE "find_arduino_library: ${lib} _hint_paths ${_hint_paths}")
    ...
    message(VERBOSE "find_arduino_library: ${lib} NOT '${parsed_args_NO_DEFAULT_PATH}'")
    ...
    if (NOT parsed_args_NO_DEFAULT_PATH)
        ...
        message(VERBOSE "find_arduino_library: _index_local_libraries(${_local_namespace} ${_ign})")
        _index_local_libraries(_local_namespace _ign)
        message(VERBOSE "find_arduino_library: _index_local_libraries _local_namespace=${_local_namespace}")
        if (NOT _local_namespace STREQUAL "")
            list(APPEND ard_libs_ns_list "${_local_namespace}")
        endif ()
        message(VERBOSE "find_arduino_library: ards_libs_global=${ards_libs_global}")

        # Add globally indexed libraries namespace
        list(APPEND ard_libs_ns_list ards_libs_global)
    endif ()

    message(VERBOSE "find_arduino_library: Search namespaces: ${lib} - ${${ard_libs_ns_list}}")
    set(_lib_name)
    message(VERBOSE "find_arduino_library: ARDUINO_LIB_${lib}_LIBNAME=${ARDUINO_LIB_${lib}_LIBNAME}")
    message(VERBOSE "find_arduino_library: ARDUINO_LIB_${lib}_PATH   =${ARDUINO_LIB_${lib}_PATH}")
    if (DEFINED ARDUINO_LIB_${lib}_LIBNAME)
        set(_lib_name "${ARDUINO_LIB_${lib}_LIBNAME}")
    elseif (DEFINED ARDUINO_LIB_${lib}_PATH)
        set(_lib_name "${lib}")
    else ()
        message(VERBOSE "find_arduino_library: _library_search_process('${ard_libs_ns_list}' '${lib}' "
        ...
        _library_search_process("${ard_libs_ns_list}" "${lib}"
                _lib_path _lib_name "${parsed_args_EXCLUDE_LIB_NAMES}"
                "${parsed_args_EXCLUDE_INCLUDE_NAMES}")
        message(VERBOSE "find_arduino_library: _library_search_process=${_lib_path}")
        if (_lib_path)
            if (NOT DEFINED ARDUINO_LIB_${_lib_name}_PATH)
                message(VERBOSE "Found Arduino Library ${_lib_name}: "
                        "${_lib_path}")
    ...
    if (NOT _lib_name)
        if (NOT ARDUINO_LIB_${_lib_name}_PATH)
            if (NOT parsed_args_QUIET)
                message(STATUS "find_arduino_library: parsed_args => ${parsed_args}")
                message(STATUS "find_arduino_library: parsed_args => ${_flag_options}")
                message(STATUS "find_arduino_library: parsed_args => ${_one_arg_options}")
                message(STATUS "find_arduino_library: parsed_args => ${_multi_arg_options}")
                message(STATUS "find_arduino_library: parsed_args => ${ARGN}")
                message(STATUS "find_arduino_library: _hint_paths '${_hint_paths}'")
                message(STATUS "find_arduino_library: ARDUINO_LIB_${lib}_LIBNAME => ${ARDUINO_LIB_${lib}_LIBNAME}")
                message(STATUS "find_arduino_library: ARDUINO_LIB_${lib}_PATH => ${ARDUINO_LIB_${lib}_PATH}")
                message(STATUS "find_arduino_library: _lib_name => ${_lib_name}")
    ...
    message(VERBOSE "find_arduino_library: parsed_args_LIBNAME_RESULT='${parsed_args_LIBNAME_RESULT}'")
    ...
    message(VERBOSE "find_arduino_library: return_lib_path='${return_lib_path}' ARDUINO_LIB_${_lib_name}_PATH='${ARDUINO_LIB_${_lib_name}_PATH}'")
...
function(target_get_arduino_lib target ret_var)
    message(VERBOSE "target_get_arduino_lib: target=${target}")
    if (NOT TARGET "${target}")
        set("${ret_var}" "" PARENT_SCOPE)
        message(VERBOSE "target_get_arduino_lib: target=${target}, RETURN '${${ret_var}}'")
        return()
    endif ()
    get_target_property(_lib_type "${target}" TYPE)
    message(VERBOSE "target_get_arduino_lib: _lib_type=${_lib_type}")
    if ("${_lib_type}" STREQUAL "INTERFACE_LIBRARY")
        set("${ret_var}" "" PARENT_SCOPE)
        message(VERBOSE "target_get_arduino_lib: _lib_type=${_lib_type}, RETURN '${${ret_var}}'")
        return()
    endif ()
    get_target_property(_ard_lib_name "${target}" ARDUINO_LIB)
    message(VERBOSE "target_get_arduino_lib: _ard_lib_name=${_ard_lib_name}")
    if (NOT _ard_lib_name)
        set("${ret_var}" "" PARENT_SCOPE)
        message(VERBOSE "target_get_arduino_lib: _ard_lib_name=${_ard_lib_name}, RETURN '${${ret_var}}'")
        return()
    endif ()
    message(VERBOSE "target_get_arduino_lib: target=${target}, FALLBACK '${_ard_lib_name}'")
    ...
    foreach (inc IN LISTS _all_includes ITEMS "Arduino")
        ...
        if (_idx LESS 0)
            if (inc STREQUAL "Arduino")
                ...
                message(VERBOSE "target_link_arduino_libraries: _map_libs_to_lib_names(${inc} ${EXCLUDE_LIB_NAMES} QUIET)")
...
function(_link_ard_lib_list target_name lib_list_var link_type
    ...
    message(VERBOSE "_link_ard_lib_list '${target_name}' '${${lib_list_var}}' '${_link_type}' '${ignore_list_var}' '${ignore_list_var}'")

    set(_link_targets)
    foreach (_lib IN LISTS ${lib_list_var})

        # Get a suitable name for the target corresponding to lib
...
function(_add_internal_arduino_library target lib)
    ...
    message(VERBOSE "_add_internal_arduino_library: parsed_args_PATH = ${parsed_args_PATH}")
...
function(_map_libs_to_lib_names libs_list_var)
    ...
    foreach (_lib IN LISTS "${libs_list_var}")
        message(VERBOSE "_map_libs_to_lib_names: search ${_lib}")
        if ("${_lib}" STREQUAL "core")
            list(APPEND _lib_names_list "${_lib}")
            message(VERBOSE "_map_libs_to_lib_names: APPEND ${_lib}")
            continue()
        endif ()
        message(VERBOSE "_map_libs_to_lib_names: target_get_arduino_lib(${_lib} _ard_lib_name)")
        target_get_arduino_lib("${_lib}" _ard_lib_name)
        message(VERBOSE "_map_libs_to_lib_names: target_get_arduino_lib '${_lib}' => '${_ard_lib_name}'")
        if (_ard_lib_name)
            message(VERBOSE "_map_libs_to_lib_names: target_get_arduino_lib '${_lib}' APPEND")
            list(APPEND _lib_names_list "${_lib}")
        else ()
            message(VERBOSE "_map_libs_to_lib_names: _lib = ${_lib}")
            message(VERBOSE "_map_libs_to_lib_names: _lib_path = ${_lib_path}")
            message(VERBOSE "_map_libs_to_lib_names: _lib_name = ${_lib_name}")
            message(VERBOSE "_map_libs_to_lib_names: ARGN = ${ARGN}")
            find_arduino_library("${_lib}" _lib_path LIBNAME_RESULT _lib_name ${ARGN})
            if (_lib_path)
                message(VERBOSE "_map_libs_to_lib_names: APPEND ${_lib_name}")
